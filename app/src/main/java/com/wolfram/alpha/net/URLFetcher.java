...
public class URLFetcher {
	...
	private static Logger logger = Logger
			.getLogger("com.wolfram.alpha.net.URLFetcher");

	// Largest result that will be allowed to be returned as a byte[] instead of
	// in a file.
	private static final int MAX_BUFFER_SIZE = 1000000;

	// TODO: outFile = null means get data as string. Improve
	// getResponseString() to be safer for large responses.
	...
	public void cancel() {
		wasCancelled = true;
		// Although we cannot abort attempts to connect, at least we can tell it
		// to not retry.
		if (trans != null)
			trans.setNoRetry();
	}
	...
	public void fetch() {
		...
		try {
			...
			try {
				...
				// Create the output stream we will write into. Will be either a
				// buf[] or a file.
				if (useFile) {
					if (outFile.length() > 0) {
						downloadedFile = new File(outFile);
					} else {
						// If user passed in "" for outFile, create one in the
						// standard temp dir.
						downloadedFile = File.createTempFile("WolframAlphaAPI",
								".tmp", null);
						outFile = downloadedFile.getAbsolutePath();
					}
					outStream = new FileOutputStream(downloadedFile);
				} else {
					// Want result in byte[] buffer. First check if it is too
					// large.
					if (contentLength == -1 || contentLength > MAX_BUFFER_SIZE)
						throw new WAException(
								"Content from URL "
										+ url
										+ " is unknown or too large to be buffered in memory. Read into a file instead.");
				...
				long maxBytesToDownload = useFile ? Long.MAX_VALUE
						: contentLength;
				int numRead;
				// TODO: Probably add Thread.wasInterrupted() test (or whatever
				// it is called), to this loop.
				while ((numRead = responseStream.read(buf)) != -1
						&& !wasCancelled) {
					totalBytesDownloaded += numRead;
					if (totalBytesDownloaded <= maxBytesToDownload)
						outStream.write(buf, 0, numRead);
				}
				// Might be useful someday to handle all the checked exceptions
				// differently...
				...
				if (outStream != null) {
					if (!wasCancelled && !useFile)
						bytes = ((ByteArrayOutputStream) outStream)
								.toByteArray();
					try {
						outStream.close();
					} catch (Exception e) {
			...
			if (exception != null) {
				logger.warning("Exception downloading URL " + url + ". "
						+ exception);
			}

			if (wasCancelled)
				logger.info("Download of URL " + url
						+ " was cancelled by user. Elapsed millis: "
