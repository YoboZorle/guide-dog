...
/**
 * Responsible for all things related to proxies. This includes storing the
 * manually-set proxy host and port (if any), deciding what proxy, if any, to
 * use for a given request (determined by manual or system settings), and
 * serving up username/password credentials if required for an authenticating
 * proxy.
 * 
 * The one thing this class does not do is handle prompting the user for
 * username and password if necessary. That is handled in Mathematica code
 * triggered by a 407 HTTP status.
 * 
 * The HttpHandler instance holds one instance of this class and uses it when
 * necessary or passes it to other classes that need to use it.
 * 
 * @author tgayley
 * 
 */
public class ProxySettings {
	...
	private ProxySettings() {
		// Try to grab system-specific proxy settings (e.g., from Internet
		// Explorer on Windows).
	...
	public synchronized void setProxyInfo(int useProxy, String httpProxyHost,
			int httpProxyPort) {
	...
	// Decide what proxy, if any, to use for a request. Manual settings take
	// precedence,
	// then we fall back to using system settings (e.g., Internet Explorer
	// settings on Windows)
	...
	public synchronized String[] getProxyHostAndPort(String url) {
		...
		if (useProxy == PROXY_AUTOMATIC) {
			ProxySelector ps = ProxySelector.getDefault(); // Will get
															// MyProxySelector.
			try {
				URI uri = new URI(url);
			...
			if (colonPos != -1) {
				...
				// Shouldn't happen, but might as well do something and let it
				// fail later.
				protocol = "http";
			}
	...
	public synchronized Proxy getProxyForJavaNet(String url) {
		...
		// If we got settings for both host and port, use them; otherwise no
		// proxy.
		if (port != 0 && host != null)
			return new Proxy(Proxy.Type.HTTP, new InetSocketAddress(host, port));
		else
			return null; // Don't return Proxy.NO_PROXY, as that seems not to be
							// a usable value--just a sentinel.
	}

	// This class is plugged in as the default system-wide ProxySelector.
	// It's a slightly weird setup, since ProxySettings calls this class for
	// Automatic
	// proxy configuration, and this class calls back to ProxySettings for
	// Manual and
	// Direct proxy settings. Things would be a little cleaner if we didn't have
	// to worry
	...
	static class MyProxySelector extends ProxySelector {
		...
		// The list we return when we want to indicate to the caller that no
		// proxy should be used.
		// In older versions of Java you could return an empty list, but in
		// newer ones there is a bug
		// where the java.net code will throw a NullPointerException if you
		...
		public List<Proxy> select(URI uri) {
			...
			if (useProxy == PROXY_AUTOMATIC)
				...
				Proxy p = (Proxy) ProxySettings.getInstance()
						.getProxyForJavaNet(uri.toString());
				if (p != null) {
