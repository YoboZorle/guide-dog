...
public class WAQueryParametersImpl implements WAQueryParameters, Serializable {
	...
	// Copy constructor.
	// When a WAEngine creates a query, it wants to pass in its
	// WAQueryParameters. But it can't pass a
	// reference to its own object because the two versions need to be isolated
	// from changes in the other.
	...
	public WAQueryParametersImpl(WAQueryParameters orig) {
		...
		// NOTE that we do no copying of the String[2] arrays. That means if you
		// modify the contents
		// of one of the arrays, the change can be seen by other
		// WAQueryParameters objects that were
		// created by copying the original. Therefore, users of the (obscure)
		// extraParams() feature
		// should take care never to alter one of these pairs in place; rather,
	...
	public double[] getLatLong() {
		if (latitude == null || longitude == null)
			...
			return new double[] { latitude.doubleValue(),
	...
	public void setLatLong(String latlong) throws IllegalArgumentException {
		...
		if (parts.length != 2)
			throw new IllegalArgumentException(
					"latlong specification must be two numbers separated by a comma");
	...
	public void addPodState(WAPodState podstate) {
		// When adding a podstate, we treat single-value and <statelist>
		// podstates differently. Single-state ones
		// just accumulate (eg. More,More,More), whereas statelist ones do not
		// (no point in doing earthquakes "last 24 hours"
		...
		if (podstate.getInputs().length > 1) {
			...
			while (iter.hasNext()) {
				...
				long id = state.getID();
				if (id == newID) {
					// If we get here, we have found an old podstate that is the
					// same one as the one we are adding (except
					// of course possibly a differently currently-selected
					// index). Replace with the new one and exit.
					iter.set(podstate);
	...
	public List<String[]> getParameters() {
		...
		if (latitude != null && longitude != null) {
			param = new String[2];
			param[0] = "latlong";
			param[1] = Double.toString(latitude) + ","
		...
		for (String a : assumptions) {
			param = new String[2];
			param[0] = "assumption";
			// Assumption strings are already encoded in the output, so not
			// encoded here.
			param[1] = a;
			params.add(param);
		}
	...
	private static String encode(String s) {
		try {
			...
			return null; // Will never happen, because UTF-8 is always
							// supported. Doesn't matter what we return.
		}
	}
	...
	public void fillFromURL(String url) {
		...
		for (String param : params) {
			...
			if (temp.length == 2) {
				try {
					...
					// Do nothing; param will be lost. But exception is
					// impossible.
				}
			}
		}
		...
		if (relatedLinks != null)
			setRelatedLinks(relatedLinks.get(0).equals("true"));
		List<String> reinterpret = parmsMap.get("reinterpret");
		if (reinterpret != null)
			setReinterpret(reinterpret.get(0).equals("true"));

		// Lump together all the calls that interpret numbers. If any of these
		// throws, we will just lose
		// setting for that param and the others that follow.
		try {
			List<String> width = parmsMap.get("width");
			if (width != null)
				setWidth(Integer.parseInt(width.get(0)));
			List<String> maxWidth = parmsMap.get("maxwidth");
			if (maxWidth != null)
				setMaxWidth(Integer.parseInt(maxWidth.get(0)));
			List<String> plotWidth = parmsMap.get("plotwidth");
			if (plotWidth != null)
				setPlotWidth(Integer.parseInt(plotWidth.get(0)));
			List<String> magnification = parmsMap.get("mag");
			if (magnification != null)
				setMagnification(Double.parseDouble(magnification.get(0)));
			...
			if (scantimeout != null)
				setScanTimeout(Double.parseDouble(scantimeout.get(0)));
			List<String> podtimeout = parmsMap.get("podtimeout");
			if (podtimeout != null)
				setPodTimeout(Double.parseDouble(podtimeout.get(0)));
			List<String> formattimeout = parmsMap.get("formattimeout");
			if (formattimeout != null)
				setFormatTimeout(Double.parseDouble(formattimeout.get(0)));
