...
public class WASubpodImpl implements WASubpod, Visitable, Serializable {
	...
	WASubpodImpl(Element thisElement, HttpProvider http, File tempDir)
			throws WAException {
		...
		for (int i = 0; i < numSubElements; i++) {
			...
			if ("plaintext".equals(name)) {
				...
				contentList
						.add(new WAImageImpl((Element) child, http, tempDir));
			}
		}
		contentElements = contentList
				.toArray(new Visitable[contentList.size()]);
	}
	...
	public void acquireImage() {
		...
		// The only synchronization needed here is that imageAcquired is
		// volatile.
