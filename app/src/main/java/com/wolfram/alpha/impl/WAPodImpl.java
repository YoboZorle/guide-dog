...
public class WAPodImpl implements WAPod, Visitable, Serializable {
	...
	WAPodImpl(Element thisElement, HttpProvider http, File tempDir)
			throws WAException {
	...
	private synchronized void createFromDOM(Element thisElement)
		...
		// The only time error=true is for a pod obtained from an async URL.
		// Normal pods are simply
		// absent if they have an error. Error pods are missing all other
		// attributes and have no subelement content.
		if (!error) {
			title = thisElement.getAttribute("title");
			scanner = thisElement.getAttribute("scanner");
			try {
				position = Integer.parseInt(thisElement
			...
			// subpods
			NodeList subpodElements = thisElement
					.getElementsByTagName("subpod");
			int numSubpods = subpodElements.getLength();
			subpods = new WASubpodImpl[numSubpods];
			for (int i = 0; i < numSubpods; i++)
				subpods[i] = new WASubpodImpl((Element) subpodElements.item(i),
						http, tempDir);

			// states
			NodeList podstatesElements = thisElement
					.getElementsByTagName("states");
			// Should be 0 or 1; this is the number of <states> elements, not
			// the number
			...
			if (numStatesElements > 0) {
				...
				// Program defensively and don't assume that every element in a
				// <states> is a <state>
				// or <statelist>, although we have no intention of making such
				// a change in the API output.
				int numSubElements = subElements.getLength();
				List<Node> stateAndStatelistNodes = new ArrayList<Node>(
						numSubElements);
				...
				for (int i = 0; i < numStates; i++)
					podstates[i] = new WAPodStateImpl(
							(Element) stateAndStatelistNodes.get(i));
			}

			// infos
			NodeList infosElements = thisElement.getElementsByTagName("infos");
			// Should be 0 or 1; this is the number of <infos> elements, not the
			// number
			...
			if (numInfosElements > 0) {
				...
				// Program defensively and don't assume that every element in an
				// <infos> is an <info>,
				// although we have no intention of making such a change in the
				// API output.
				int numSubElements = subElements.getLength();
				...
				for (int i = 0; i < numInfos; i++)
					infos[i] = new WAInfoImpl((Element) infoNodes.get(i), http,
							tempDir);
			}
			...
			if (numSoundElements > 0) {
				...
				for (int i = 0; i < numSounds; i++)
					sounds[i] = new WASoundImpl((Element) soundNodes.get(i),
							http, tempDir);
			}
	...
	public void finishAsync() throws WAException {
		...
		// This is structured so that it holds the synchronization lock for the
		// shortest possible period
		// of time--only when reading or setting instance state. Do not want to
		// hold the lock during download
		// of async pods. Note that two different threads could call
		// finishAsync() at close to the same time
		// and trigger two separate downloads. This is not really a problem. The
		// synchronization issue we care
		// about is making sure that changes made by a thread calling
		// finishAsync() are immediately seen by
		...
		if (url != null) {
			try {
				...
				if (fetcher.wasCancelled())
					throw new WAException("Download of url " + asyncURL
							+ " was cancelled");
	...
	// It is not essential that access to userData be synchronized here. Users
	// could synchronize on their.
	// But the Android app is the main client for this feature, and the Android
	// app needs synchronization, so
	// I'll put it in here.
	...
	// We use hashCode() as a "content code" to tell us quickly whether the
	// object's content
	// has changed since some point in the past. Note that we do not override
	// equals() as well,
	// but it is not necessary to override equals() when overriding hashCode()
	// (although it _is_
	// necessary to override hashCode() when overriding equals()). Every field
	// on which the
	// hashcode depends must be either immutable or volatile (or otherwise
	// controlled by synchronized
	// blocks), as these values are changed on a background thread.
	// This is not a particularly good hash function, but it doesn't need to be.
	// The only property
	// that really matters is that its value changes when the content of this
	// object changes.
	...
	public synchronized int hashCode() {
		...
		// The only content that can change in this class is the error state and
		// asyncException (these from
		// an async content download), the userData, and the contents of the
		// subpods and the
		// sounds. Both of those objects can change as their content is
		// downloaded (e.g., image and sound URLs)
		// after initial creation of the object. Since hashCode() is currently
		// used only for layout
		// considerations, we don't really care about sounds (their visual
		// representation on screen is not
		// affected by whether the sound file has been downloaded or not), but
