...
public class WAQueryResultImpl implements WAQueryResult, Visitable,
		Serializable {
	...
	// TODO: Provide soem way to release the bytes array. The only reason to
	// keep it around is for
	// the getXML() method, but many users will never want to call that. Perhaps
	// the ctor can take an
	// arg that says whether to store the byte array or not.

	// I _could_ lazily parse the bytes, but I think it's better to force
	// parsing errors to
	// occur in a well-defined place, not potentially from every accessor.
	public WAQueryResultImpl(WAQuery query, byte[] bytes, HttpProvider http,
			File tempDir) throws WAException {
		...
		try {
			/***
			 * OLD SAX style, abandoned SAXParserFactory factory =
			 * SAXParserFactory.newInstance(); SAXParser parser =
			 * factory.newSAXParser(); AlphaSAXHandler handler = new
			 * AlphaSAXHandler(this); parser.parse(new
			 * ByteArrayInputStream(bytes), handler);
			 ***/
			...
			// NodeList queryResultNode =
			// doc.getElementsByTagName("queryresult");
			...
			// Don't think there can be an IOException on a
	...
	// Can be null (if this is a recalc query, performed by
	// WAEngine.performReclaculate(), or a deserialized instance).
	...
	// Because the pods array is modified in the merge methods, which are called
	// on threads, we must ensure
		...
		if (!imagesAcquired) {
			for (WAPodImpl pod : pods) {
				try {
					...
					// What to do here? Need to finish getting all even if there
					// is an exception.
				}
			}
			imagesAcquired = true;
	...
	// These are used for updating a WAQueryResult with info from another
	// result, either a podstate chnage or a recalculate.
	// They don't keep the object in a totally consistent state. For example,
	// calling getXML() after a merge will not
	// give you updated XML. These are just convenience functions for clients
	// who don't want to know the correct way
	// to merge in these new types of results. The "important" getters, like
	// getPods(), work as desired after a merge.

	public synchronized void mergeRecalculateResult(
			WAQueryResult recalcQueryResult) {

		// TODO: Merge in new <sources> or other elements that are relevant in a
		// recalc.
		WAPod[] recalcPods = recalcQueryResult.getPods();
		WAPod[] oldPods = getPods();
		if (recalcPods.length > 0) {
			WAPodImpl[] allPods = new WAPodImpl[oldPods.length
					+ recalcPods.length];
			System.arraycopy(oldPods, 0, allPods, 0, oldPods.length);
			System.arraycopy(recalcPods, 0, allPods, oldPods.length,
					recalcPods.length);
			pods = allPods;
		}
	}

	public synchronized void mergePodstateResult(
			WAQueryResult podstateQueryResult) {
	...
	public void release() {
		// Could do this like acquireImages, via explicit interface methods, or
		// by a visitor.
		// Probably via visitor, since it doesnt seem likely that a user would
		// want to release
		// a specific pod. Probably just want to throw away everything when done
		// with a query.
	}
	...
	private void createFromDOM(Element thisElement) throws WAException {
		...
		if (error) {
			// Error case is missing many attributes and has different
			// subelement content.
			// Format of error element: <error><code>42</code><msg>blah
			// blah</msg></code>.
			...
			if (children.getLength() > 0) {
				...
				if (children.getLength() > 0) {
					try {
						errorCode = Integer.parseInt(children.item(0)
								.getFirstChild().getNodeValue());
						children.item(0).getFirstChild();
					} catch (NumberFormatException e) {
						// Do nothing in this unlikely event. errorCode remains
						// its default value of 0 ("no error").
					}
				}
				children = errorElement.getElementsByTagName("msg");
				if (children.getLength() > 0) {
					errorMessage = children.item(0).getFirstChild()
							.getNodeValue();
				}
			}
		} else {
			...
			try {
				parseTiming = Double.parseDouble(thisElement
						.getAttribute("timing"));
			} catch (NumberFormatException e) {
			}
			...
			for (int i = 0; i < numPods; i++)
				pods[i] = new WAPodImpl((Element) podElements.item(i), http,
						tempDir);

			NodeList assumptionElements = thisElement
					.getElementsByTagName("assumption");
			int numAssums = assumptionElements.getLength();
			assumptions = new WAAssumptionImpl[numAssums];
			for (int i = 0; i < numAssums; i++)
				assumptions[i] = new WAAssumptionImpl(
						(Element) assumptionElements.item(i));

			NodeList warningsElements = thisElement
					.getElementsByTagName("warnings");
			// There should be 0 or 1. This is the <warnings> element, not the
			// elements for each
			...
			if (warningsElements.getLength() > 0) {
				...
				for (int i = 0; i < numWarnings; i++) {
					...
					if ("reinterpret".equals(warningElement.getNodeName()))
						warnings[i] = new WAReinterpretWarningImpl(
								warningElement);
					else
			...
			// There should be 0 or 1. This is the <tips> element, not the
			// elements for each
			// individual <tip>.
			if (tipsElements.getLength() > 0) {
				...
				for (int i = 0; i < numTips; i++)
					splatTips[i] = new String(tipElements.get(i).getAttribute(
							"text"));
			}

			NodeList relatedLinkElements = thisElement
					.getElementsByTagName("sidebarlink");
			int numRelateds = relatedLinkElements.getLength();
			relatedLinks = new WARelatedLinkImpl[numRelateds];
			for (int i = 0; i < numRelateds; i++)
				relatedLinks[i] = new WARelatedLinkImpl(
						(Element) relatedLinkElements.item(i), http, tempDir);

			NodeList didYouMeanElements = thisElement
					.getElementsByTagName("didyoumean");
			...
			NodeList relatedExampleElements = thisElement
					.getElementsByTagName("relatedexample");
			...
			if (numRelatedExamples > 0) {
				...
				for (int i = 0; i < numRelatedExamples; i++)
					relatedExamples[i] = new WARelatedExampleImpl(
							(Element) relatedExampleElements.item(i), http,
							tempDir);
			}

			NodeList languageMsgElements = thisElement
					.getElementsByTagName("languagemsg");
			int numLanugageMsgs = languageMsgElements.getLength();
			// Should be 0 or 1.
			if (numLanugageMsgs > 0) {
				Element languageMsgElement = (Element) languageMsgElements
						.item(0);
			...
			NodeList sourcesElements = thisElement
					.getElementsByTagName("sources");
			// There should be 0 or 1. This is the <sources> element, not the
			// elements for each
			// individual <source>.)
			if (sourcesElements.getLength() > 0) {
				...
				for (int i = 0; i < numSources; i++)
					sources[i] = new WASourceInfoImpl(
							(Element) sourceElements.get(i));
			}

			NodeList futureTopicElements = thisElement
					.getElementsByTagName("futuretopic");
			// There should be 0 or 1.
			if (futureTopicElements.getLength() > 0)
				futureTopic = new WAFutureTopicImpl(
						(Element) futureTopicElements.item(0));

			NodeList examplePageElements = thisElement
					.getElementsByTagName("examplepage");
			// There should be 0 or 1.
			if (examplePageElements.getLength() > 0)
				examplePage = new WAExamplePageImpl(
						(Element) examplePageElements.item(0));
		}
	}
