...
public class WAAssumptionImpl implements WAAssumption, Visitable, Serializable {
	...
	WAAssumptionImpl(Element thisElement) throws WAException {
		...
		if (description.equals(""))
			description = null;
		// These two will fall back to their default values if the attributes
		// are not present. In the case of 'count' that
		...
		for (int i = 0; i < numValueElements; i++) {
			...
			valids[i] = value.getAttribute("valid").equals("false") ? false
					: true;
		}
	}
