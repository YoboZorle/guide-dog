...
public interface WAQueryParameters {
	...
	// Values < 0 mean async off, 0 means async=true (default timeout), > 0
	// means async is that value.
	double getAsync();
	...
	// This doesn't really work for <statelist>-type states. The query will
	// work, but if you modify an existing
	// query, you won't get the new state replacing the old state. Instead, you
	// get chaining, so the old replacement happens
	// followed by the new replacement. Probably still works, but it's
	// inefficient. (I'm not sure if the order is guaranteed,
	// so the old state might still be in effect.)
	void addPodState(String podstate);

	// This one is for <statelist> types, but not intended to be called by
	// users, since they don't know what the id is.
