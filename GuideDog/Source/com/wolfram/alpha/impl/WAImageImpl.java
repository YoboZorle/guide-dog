...
public class WAImageImpl implements WAImage, Visitable, Serializable {
	...
	WAImageImpl(Element thisElement, HttpProvider http, File tempDir)
			throws WAException {
	...
	// This ctor for use when not being created from an <img> element, like for
	// the thumbnail images in WARelatedExample.
	...
	public void acquireImage() {
		...
		// If this is a deserialized instance, http will be null. Such instances
		// are "dead"; they can
		...
		if (!imageAcquired && http != null) {
			try {
				...
				if (format == WAImage.FORMAT_GIF)
					...
					suffix = ".tmp";
				String outFile = File
						.createTempFile("WAImage", suffix, tempDir)
						.getAbsolutePath();
				URLFetcher fetcher = new URLFetcher(new URL(url), outFile,
						http, null);
				fetcher.fetch();
	...
	synchronized void setFile(File file) {
		this.file = file;
		cachedHashCode = 0; // Force recompute of hash now that content has
							// changed.
	}
