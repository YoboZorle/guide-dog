...
public class WAPodStateImpl implements WAPodState, Visitable, Serializable {
	...
	// Because all podstates stored in a WAQueryParameters are represented as
	// WAPodStates, there is a need
	// to create a "dummy" podstate from just an input string. This is used by
	// the addPodState(String) signature.
	...
	private synchronized void createFromDOM(Element thisElement)
		...
		if ("state".equals(nodeName)) {
			...
			// Old-style API results only have a name and not an input
			// attribute. Support old API
			// by using name value as input value. This is probably not
			// important, as by the time this
			// library is in use, all API servers will be running new-style
			// code.
			if ("".equals(input))
				input = name;
			...
			// <statelist> is a <state>,
			// although we have no intention of making such a change in the API
			// output.
