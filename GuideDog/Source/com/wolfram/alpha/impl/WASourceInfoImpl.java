...
public class WASourceInfoImpl implements WASourceInfo, Serializable {
	...
	WASourceInfoImpl(Element thisElement) throws WAException {
		...
		if (url.endsWith("SourceInformationNotes.html")) {
			String dataType = url.substring(url.lastIndexOf('/') + 1,
					url.length() - "SourceInformationNotes.html".length());
			formattedUrl = "http://www.wolframalpha.com/input/sources.jsp?sources="
					+ dataType;
		} else {
			formattedUrl = url;
		}
